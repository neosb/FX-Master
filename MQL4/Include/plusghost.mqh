...
bool GhostFreeSelect(bool incr=true)
{
    bool ret;
...
//|-----------------------------------------------------------------------------------------|
int GhostTerminalRefresh(double Pts)
{
...
int GhostLabel(string labelName, int xDist, int yDist, int labelCorner=0)
{
    int lastErr;
	...
	for ( int i = GhostCurOpenPositions - 1; i >= 0; i-- )
	{
		for ( int j = GhostCurOpenPositions - 1; j >= 0; j-- )
		{
			if ( StrToInteger( GhostOpenPositions[i][0] ) > StrToInteger( GhostOpenPositions[j][0] ) )
			{
				...
				for ( int n = 0; n < 11; n ++ ) { GhostOpenPositions[i][n] = GhostOpenPositions[j][n]; }
				for ( int n = 0; n < 11; n ++ ) { GhostOpenPositions[j][n] = tmp[n]; }
			}
		}
	}
	for ( int i = GhostCurPendingOrders - 1; i >= 0; i-- )
	{
		for ( int j = GhostCurPendingOrders - 1; j >= 0; j-- )
		{
			if ( StrToInteger( GhostPendingOrders[i][0] ) > StrToInteger( GhostPendingOrders[j][0] ) )
			{
				for ( int n = 0; n < 11; n ++ ) { tmp[n] = GhostPendingOrders[i][n]; }
				for ( int n = 0; n < 11; n ++ ) { GhostPendingOrders[i][n] = GhostPendingOrders[j][n]; }
				for ( int n = 0; n < 11; n ++ ) { GhostPendingOrders[j][n] = tmp[n]; }
			}
		}
	}
}
